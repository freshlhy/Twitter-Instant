...
- (void)viewDidLoad {
    ...
    [[self.searchText.rac_textSignal map:^id(NSString *text) {
        return [self isValidSearchText:text] ? [UIColor whiteColor]
                                             : [UIColor yellowColor];
    }]
    ...
    [[[[[[[self requestAccessToTwitterSignal] then:^RACSignal * {
        ...
        error:^(NSError *error) { NSLog(@"An error occurred: %@", error); }];
}
...
- (RACSignal *)requestAccessToTwitterSignal {
    ...
    // 1 - define an error
    NSError *accessError =
        [NSError errorWithDomain:RWTwitterInstantDomain
...
- (SLRequest *)requestforTwitterSearchWithText:(NSString *)text {
    NSURL *url =
        [NSURL URLWithString:@"https://api.twitter.com/1.1/search/tweets.json"];
